...
extern "C" void evaluate_first_order_h_bias_part2_(
    int *j_atom_begin_,
    int *j_atom_end_,
    // scalars
    int *n_full_points_,
    ...
    for (int j_atom = j_atom_begin; j_atom <= j_atom_end; j_atom++) {
      ...
      if (n_compute_c > 0) {
        ...
        for (int i_point = 0; i_point < n_points; ++i_point) {
          ...
          double prefactor4_1 = partition_tab(i_full_point);
          double prefactor4_2 = partition_tab(i_full_point) * vrho(i_point);
          double prefactor4_3[3];

          for (int i = 0; i < 3; ++i) {
            prefactor4_3[i] = partition_tab(i_full_point) * 2.0 * vsigma(0, i_point) * gradient_rho(i, i_full_point);
          }
          for (int i_compute = 0; i_compute < n_compute_c; ++i_compute) {
            double contract_4 = 0;
            contract_4 += prefactor4_1 * H_times_psi(i_compute, i_point);
            contract_4 += prefactor4_2 * wave(i_compute, i_point);
            for (int i = 0; i < 3; i++) {
              contract_4 += prefactor4_3[i] * gradient_basis_wave(i_compute, i, i_point);
            }
            contract_sum_2(i_compute, i_point) = contract_4;
          }
        }

        local_first_order_H.setZero(); // TODO 删除
        ...
        if (valid_n_compute_count > 0) {
          ...
          // TODO 优化为 1 个 n_compute_c * valid_n_compute_count * n_points 矩阵乘法 + 转置
          // 但是此前需要比较选出哪个矩阵乘更快
          // 目前只更新下三角，所以 n_compute_c 对应的维度只用了一半

          // TODO 提前存储结果，由于不确定直接存最终结果带来的内存负担，暂时没做
          // contract_sum_2 是 n_compute_c * n_points 常量矩阵，与 cpscf 迭代、j_atom 均无关
          // first_order_wave 是 valid_n_compute_count * n_points 常量矩阵，但由 j_atom  决定
          // 结果矩阵是 n_compute_c * valid_n_compute_count 常量矩阵
          ...
          cblas_dgemm(
              ...
              n_compute_c,
              0.0,
              &local_first_order_H(0, valid_i_compute_start),
              n_compute_c);

          for (int j_compute = valid_i_compute_start; j_compute < valid_i_compute_end; j_compute += 1) {
            for (int i_compute = 0; i_compute < n_compute_c; i_compute += 1) {
              ...
              first_order_H(i_basis, j_basis) += local_first_order_H(i_compute, j_compute);
              first_order_H(j_basis, i_basis) += local_first_order_H(i_compute, j_compute);
            }
          }
        }
        ...
        // for (int i_compute = 0; i_compute < n_compute_c; i_compute += 1) {
        //   int i_basis = i_basis_batches(i_compute, i_my_batch) - 1;
        //   first_order_H(i_basis, i_basis) = first_order_H(i_basis, i_basis) + local_first_order_H(i_compute,
        //   i_compute); for (int j_compute = i_compute + 1; j_compute < n_compute_c; j_compute += 1) {
        //     int j_basis = i_basis_batches(j_compute, i_my_batch) - 1;
