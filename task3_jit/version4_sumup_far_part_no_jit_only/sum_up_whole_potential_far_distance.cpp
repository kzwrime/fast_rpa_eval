    ...
    for (int i_center = 0; i_center < n_centers_hartree_potential; i_center++) {
      ...
      for (int i_valid_points = 0; i_valid_points < n_valid_points; i_valid_points++) {
        ...
        if (dist_tab_sq < multipole_radius_sq(current_spl_atom, j_atom_inner)) {
          ...
          // 针对测试需要的体系，仅支持 l_pot_max <= 6 即可
          if (l_pot_max <= 6) {
            SHEval_s(l_atom_max, trigonom_tab[0], trigonom_tab[1], trigonom_tab[2], trigonom_tab[3], ylm_tab.data());
          } else {
            int zero = 0;
#ifndef DIRECT_TEST
            increment_ylm_(
          ...
          delta_v_hartree(i_full_points, j_atom_inner) += spline_vector_v2_n2_c_fused_ddot(
              i_r_log,
              l_h_dim,
              (l_pot_max + 1) * (l_pot_max + 1),
              n_hartree_grid,
              n_grid_ptr[species_center_ptr[current_center] - 1],
              &current_delta_v_hart_part_spl_tile(0, 0, 0, current_spl_atom, j_atom_inner),
              &ylm_tab(0));
