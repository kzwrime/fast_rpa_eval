...
float call_with_read_data(HartreePotentialData &sumup_data, FirstOrderRhoMetaData &rho_data, const char *base_path) {
  ...
  cTMi32<2> l_hartree_max_far_distance(sumup_data.l_hartree_max_far_distance.data(), sumup_data.n_atoms, ATOM_TILE_SIZE);
  cTMf64<2> multipole_radius_sq(sumup_data.multipole_radius_sq.data(), sumup_data.n_atoms, ATOM_TILE_SIZE);
  cTMf64<3> outer_potential_radius(sumup_data.outer_potential_radius.data(), sumup_data.l_pot_max + 1, sumup_data.n_atoms, ATOM_TILE_SIZE);
  cTMf64<3> multipole_moments(sumup_data.multipole_moments.data(), l_pot_max_pow2, sumup_data.n_atoms, ATOM_TILE_SIZE);
  ...
  for (int iter = 0; iter < warm_up; iter++) {
    EventHelper<true> event_function_all(stream);
    // event_function_all.record_start();
    sum_up_whole_potential_c_v3_atoms_full_points_j_atom_tile_cu_host_(
        sumup_data.j_atom_begin,
        sumup_data.j_atom_end,
        sumup_data.n_full_points,
        ...
        sumup_data.partition_tab.empty() ? nullptr : sumup_data.partition_tab.data(),
        delta_v_hartree.data());
    // event_function_all.elapsed_time(
    //     "sum_up_whole_potential_c_v3_atoms_full_points_j_atom_tile_cu_host_far_distance finished.");
  }
  ...
  float milliseconds = event_helper_all.elapsed_time("Run sum_up_whole_potential_far_distance finished.");
  // printf("Run %s sum_up_whole_potential_far_distance avg time (ms): %f\n", base_path, milliseconds / run_iters);
